...
function FullscreenCard({ card, onClose }: { card: typeof TRANSLATIONS[0]; onClose: () => void }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={`${card.lang} emergency translation card`} style={{ position: "fixed", inset: 0, background: "#fff", zIndex: 200, display: "flex", flexDirection: "column", padding: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        ...
        <button aria-label="Close fullscreen card" onClick={onClose} style={{ background: "#f3f4f6", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 600, cursor: "pointer" }}>Close</button>
      </div>
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", background: "#f9fafb", minHeight: "100vh", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ padding: "16px 16px 80px" }}>
        <div style={{ background: "#fff7ed", borderRadius: 12, padding: "10px 12px", marginBottom: 14, border: "1px solid #fdba74" }}>
          <p style={{ margin: 0, fontSize: 12, color: "#9a3412", lineHeight: 1.5 }}>
            Travel tips are guidance only, not a medical guarantee. Confirm ingredients and cross-contamination protocol with staff every time.
          </p>
        </div>
