  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link to="/trips/new">
          <Button><Plus className="mr-1.5 h-4 w-4" /> Plan new trip</Button>
        </Link>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="mt-12 rounded-3xl border border-dashed border-border bg-cream/40 p-12 text-center">
          ...
          <Link to="/trips/new">
            <Button className="mt-4"><Plus className="mr-1.5 h-4 w-4" /> Plan your first trip</Button>
          </Link>
        </div>
      ) : (
        <div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {(trips || []).map((t) => (
            <Link
              ...
              to="/trips/$id"
              params={{ id: t.id }}
              className="rounded-3xl border border-border bg-card p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-glow"
            >
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-2xl">{flagFor(t.destination_country || t.destination)}</p>
                  <h3 className="mt-2 font-display text-lg">
                    {t.title || t.destination_city || t.destination_country || t.destination}
                  </h3>
                  <p className="text-xs text-muted-foreground">
                    {t.destination_city || ""}
                    {t.destination_city && t.destination_country ? ", " : ""}
                    {t.destination_country || t.destination || ""}
                  </p>
                </div>
                <Badge
