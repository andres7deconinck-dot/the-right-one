...
import { ArrowRight, Wheat, Globe2, MessagesSquare, Map, Shield, Sparkles, Star, CreditCard, ScanLine, Lock, Zap, CheckCircle2, XCircle, AlertTriangle, Loader2, ChevronRight, MapPin } from "lucide-react";
...
const HOME_FAQ_SCHEMA = JSON.stringify({
  "@context": "https://schema.org",
  "@type": "FAQPage",
  mainEntity: [
    { "@type": "Question", name: "Is GlutenGo safe for celiac disease — not just gluten intolerance?", acceptedAnswer: { "@type": "Answer", text: "Yes. GlutenGo is built specifically for celiac disease. Translation cards mention cross-contamination explicitly, country guides flag hidden risks, and the AI assistant always prioritises medical-grade safety over convenience." } },
    { "@type": "Question", name: "Which languages are supported for translation cards?", acceptedAnswer: { "@type": "Answer", text: "GlutenGo supports 30+ languages including Italian, Japanese, Thai, Spanish, French, German, Dutch, Portuguese, Greek, Korean, Chinese (Simplified), Arabic, Hindi and more. New languages are added regularly." } },
    { "@type": "Question", name: "Does GlutenGo work offline?", acceptedAnswer: { "@type": "Answer", text: "Yes. Country guides and emergency phrases are cached locally after your first visit, so they remain accessible without an internet connection — essential when travelling in remote areas." } },
    { "@type": "Question", name: "Is there a free plan?", acceptedAnswer: { "@type": "Answer", text: "GlutenGo is free to start with 3 translation cards per month and access to all country guides. Paid plans unlock unlimited cards, offline PDF export, and priority support." } },
    { "@type": "Question", name: "How accurate are the gluten-free translations?", acceptedAnswer: { "@type": "Answer", text: "Cards are generated by an AI system tuned specifically for celiac disease, including cross-contamination wording and phonetic guides for languages like Japanese and Thai. For critical trips, we recommend having a native speaker review the card as well." } },
    { "@type": "Question", name: "Can GlutenGo be used for gluten intolerance as well as celiac disease?", acceptedAnswer: { "@type": "Answer", text: "Yes. When you create your account you can specify your severity level — celiac disease, gluten intolerance, or wheat allergy. Translation cards and AI responses adapt to your chosen severity." } },
    { "@type": "Question", name: "Which countries have the best celiac disease awareness?", acceptedAnswer: { "@type": "Answer", text: "Based on our country guides and traveller community, Italy, Portugal, Ireland, Argentina, and Sweden consistently rank highest for celiac awareness. Italy has a national law mandating celiac screening; Argentina legally recognises celiac as a disability." } },
    { "@type": "Question", name: "Can I cancel my GlutenGo subscription anytime?", acceptedAnswer: { "@type": "Answer", text: "Yes. Cancel from your account settings at any time. You keep full access until the end of your paid billing period. All paid plans include a 30-day money-back guarantee." } },
  ],
});
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "GlutenGo: Gluten-Free Travel App for Celiac Disease" },
      { name: "description", content: "Find certified gluten-free restaurants, generate celiac translation cards in 30+ languages, and explore country guides for 26 countries. Built specifically for celiac disease — cross-contamination warnings included. Free to start." },
    ...
    scripts: [
      { type: "application/ld+json", children: HOME_FAQ_SCHEMA },
    ],
  }),
  component: Landing,
});
...
function Landing() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SiteHeader />
      {/* Hero */}
      <section className="relative overflow-hidden bg-hero" aria-label="GlutenGo hero">
      ...
      <div className="border-b border-border bg-card/60 backdrop-blur">
        <div className="mx-auto max-w-7xl px-5 py-3">
          <div className="flex flex-wrap items-center justify-center gap-x-7 gap-y-2 text-sm text-muted-foreground">
            <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-primary" /><strong className="text-foreground">{COUNTRIES.length} country</strong> guides</span>
            <span className="hidden h-3 w-px bg-border sm:block" />
            <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-primary" /><strong className="text-foreground">30+</strong> languages</span>
            <span className="hidden h-3 w-px bg-border sm:block" />
            <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-primary" />Cross-contamination warnings included</span>
            <span className="hidden h-3 w-px bg-border sm:block" />
            <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-primary" />Works offline</span>
            <span className="hidden h-3 w-px bg-border sm:block" />
            <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-primary" />Free to start</span>
          </div>
        </div>
      </div>
      ...
      {/* Celiac-safe cities */}
      <section className="mx-auto max-w-7xl px-5 py-16" aria-label="Top celiac-friendly destinations">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Top destinations for celiacs</p>
            <h2 className="mt-2 font-display text-3xl md:text-4xl">Where celiacs travel safely</h2>
          </div>
          <Link to="/countries" className="hidden shrink-0 items-center gap-1 text-sm font-medium text-primary hover:underline sm:flex">
            All {COUNTRIES.length} country guides <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        </div>
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
          {FEATURED_CITIES.map((city) => (
            <Link
              ...
              className="group flex flex-col items-center rounded-2xl border border-border bg-card p-4 text-center shadow-soft transition hover:-translate-y-1 hover:border-primary/40 hover:shadow-glow"
            >
              <span className="text-4xl">{city.flag}</span>
              <p className="mt-2 text-sm font-semibold leading-tight text-foreground">{city.city}</p>
              ...
              <span className="mt-2 inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700">
                <MapPin className="h-2.5 w-2.5" /> High awareness
              </span>
            </Link>
          ))}
        </div>
        <Link to="/countries" className="mt-5 flex items-center justify-center gap-1 text-sm font-medium text-primary hover:underline sm:hidden">
          See all {COUNTRIES.length} country guides <ArrowRight className="h-3.5 w-3.5" />
        </Link>
      </section>
...
const FEATURED_CITIES = [
  { flag: "🇮🇹", city: "Rome", country: "Italy", slug: "italy" },
  { flag: "🇵🇹", city: "Lisbon", country: "Portugal", slug: "portugal" },
  { flag: "🇪🇸", city: "Barcelona", country: "Spain", slug: "spain" },
  { flag: "🇮🇪", city: "Dublin", country: "Ireland", slug: "ireland" },
  { flag: "🇳🇱", city: "Amsterdam", country: "Netherlands", slug: "netherlands" },
  { flag: "🇭🇺", city: "Budapest", country: "Hungary", slug: "hungary" },
  { flag: "🇦🇷", city: "Buenos Aires", country: "Argentina", slug: "argentina" },
  { flag: "🇬🇷", city: "Athens", country: "Greece", slug: "greece" },
];
